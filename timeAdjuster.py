...
def main():
    while True:
        ...
        while True:
            try:
                while time_count < total_count:
                    ...
                    res = requests.get(url = time_url,headers = headers, timeout = 0.5)
                    print("text:", res.text)
                    if int(source) == 1:
                        ...
                        remote_time = float(res.json()["data"]["splash_request_id"][0:13]) #B站
                    d = remote_time - time_timer * 1000
                    print("ServerTime:", remote_time, "-", "LocalTime", time_timer, "=", d, "\n")
                    if d < max:
                        max = d
                    time.sleep(0.2)
                    time_count += 1
        ...
        print("\n时间修改成功:", time.time(), time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())))
        os.system("pause")
